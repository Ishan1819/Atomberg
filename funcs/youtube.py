...
def compute_sov(summary):
    ...
    mention_sov = (atomberg_mentions / total_mentions * 100) if total_mentions > 0 else 0
    ...
    return {
        "mention_sov(%)": round(mention_sov, 2),
        "atomberg_sentiment": round(atomberg_sent, 3),
        "others_sentiment": round(others_sent, 3),
    }

# --------- Main ---------
def youtube_search(query, num_results):
    """Fetch, analyze, and return YouTube search results with insights."""
    videos = fetch_youtube_results(query, num_results)

    print(f"🔍 Found {len(videos)} YouTube videos for query '{query}'")
    for i, vid in enumerate(videos, 1):
        title = vid.get("title", "No title")
        link = vid.get("link", "No link")
        print(f"{i}. {title}\n   {link}")
